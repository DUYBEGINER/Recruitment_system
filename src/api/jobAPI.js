...
const jobAPI = {
  ...
  /**
   * Lấy danh sách jobs với filter (cho employer - HR/TPNS)
   */
  getAllJobsByEmployer: async (params = {}) => {
    const res = await axiosClient.get('/jobs/filtered', { params });
    return res.data;
  },

  getJobByIdEmployer: async (id) => {
    const res = await axiosClient.get(`/jobs/employer/${id}`);
    return res.data;
  },
